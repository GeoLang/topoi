...
- **Measurements**: area, signed area, length, distance, and `centroid` for any geometry. In a mixed collection the centroid comes from the polygons alone, or the lines when there are no polygons
...
- **Delaunay triangulation**: Bowyer-Watson, with the Voronoi vertices (triangle circumcenters)
...
- **Validity**: `validate` reports too few points, repeated vertices, ring self-intersection and holes reaching outside their shell. `make_valid` runs polygons through a self-overlay, which splits a bowtie into a multipolygon and normalizes winding, and drops repeated vertices from lines
...
- **WebAssembly SDK**: `topoi-wasm` crate exposing convex hull, buffer, clip, split, Delaunay, simplify, point-in-polygon, polygon intersection test, boolean overlay, and bounding box to JavaScript via `wasm-bindgen`, plus a GeoJSON-string binding for every FeatureCollection op (`fcBuffer`, `fcDissolve`, `fcOverlay` and the rest), taking and returning whole collections as strings
...
`cargo install --path crates/topoi-cli` builds the `topoi` binary. Tagged
releases attach it for Linux and macOS on x86_64 and aarch64.
